...
def build_manifest_replacements(sid, po_summary, carrier_name, scac, shipment_rows):
    ...
    replacements = {
        ...
        "{{SCAC}}": scac,
        "{{TOTWEIGHT}}": shipment_rows['Weight'].iloc[0],
        "{{TOTCUBE}}": shipment_rows['Cube'].iloc[0],
...
def _get_row_data_for_po(po_row):
    return {
        ...
        'qty': '',
        'haz': '',
        'weight': str(int(po_row['Gross weight'])),
        'cube': str(int(po_row['Cube_po'])),
        'pct_truck': '',
        'case': '',
        'exp_pallets': '',
        'rec_pallets': '',
...
if missing_assets:
    st.error("Missing required asset files in /assets:")
...
if run:
    ...
    try:
        with st.spinner("Processing files and generating BOLs..."):
            df_bol, upload_merged = process_bol_files(planex_files, order_files)
        ...
        k1.metric("Generated DOCX files", len(doc_buffers))
        k2.metric("Unique Shipments", df_bol['SID'].nunique())
        k3.metric("Merged Rows", len(upload_merged))
        ...
        with st.expander("Merged Detail"):
            st.dataframe(upload_merged, use_container_width=True)
